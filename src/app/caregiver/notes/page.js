...
export default function CaregiverNotesPage() {
  ...
  return (
    <AppLayout title={t('doctorNotes')} requiredRole="caregiver">
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto">
            {['all', 'recommendation', 'medication', 'comment', 'alert_response'].map(type => (
              <button
                key={type}
                onClick={() => setTypeFilter(type)}
                className={`px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${
                  typeFilter === type ? 'bg-primary text-white' : 'bg-muted text-text-light hover:bg-slate-200'
                }`}
              >
                {type === 'all' ? t('all') : type === 'recommendation' ? t('recommendation') : type === 'medication' ? t('medicationChange') : type === 'comment' ? t('generalComment') : t('alertResponse')}
              </button>
            ))}
          </div>
        </div>
                    ...
                    <div key={note.id} className="bg-white rounded-xl border border-border p-4">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium border ${cfg.badge}`}>{cfg.label}</span>
